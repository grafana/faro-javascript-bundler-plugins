...
To install the esbuild plugin with `yarn`, run:
...
- `outputPath: string` *optional*: Override the output directory path where source maps are located, by default uses esbuild's `outdir` or `outfile` directory
